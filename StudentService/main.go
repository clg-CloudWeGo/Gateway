...
func main() {
	...
	s.InitDB()
	svr := InitEtcdRegistry(s, "StudentService", add)
	err := svr.Run()
	if err != nil {
		log.Println(err.Error())
	}
}
func InitEtcdRegistry(s *StudentServiceImpl, serviceName string, addr *net.TCPAddr) server.Server {
	...
	svr := demo.NewServer(s, server.WithRegistry(r), server.WithServiceAddr(addr), server.WithServerBasicInfo(ebi))
	return svr
}
